...
use super::inline::{FontIdentifierAndMetrics, InlineFormattingContextState};
...
//
//  1. The line breaker told us that a break was necessary between to separate
//     instances of sending text to it.
//  2. We are following replaced content ie `have_deferred_soft_wrap_opportunity`.
//
// In both cases, we don't want to do this if the first character
#[derive(PartialEq)]
...
pub(crate) struct TextRunSegment {
    /// The index of this font in the parent [`TextRun`]'s collection of font information.
...
impl TextRunSegment {
    ...
    fn is_compatible(
        ...
        font: &FontRef,
        script: Script,
        fonts: &[FontIdentifierAndMetrics],
    ) -> bool {
        fn is_specific(script: Script) -> bool {
            script != Script::Common && script != Script::Inherited
        }
        (script == self.script || !is_specific(script) || !is_specific(self.script)) &&
            font.borrow().identifier() == fonts[self.font_index].identifier
    }
...
impl TextRun {
    ...
    pub(super) fn break_and_shape(
        ...
        font_cache: &mut Vec<FontIdentifierAndMetrics>,
    ) {
        let break_results = self.break_text(font_context, font_cache);
    ...
    fn break_text(
        ...
        for (byte_index, character) in self.text.char_indices() {
            ...
            if matches!(
                current,
                Some(ref current) if current.0.is_compatible(&font, script, font_cache)
            ) {
                continue;
            }
...
pub(super) fn add_or_get_font(
    font: &FontRef,
    ifc_fonts: &mut Vec<FontIdentifierAndMetrics>,
) -> usize {
    let font = font.borrow();
    for (index, ifc_font_info) in ifc_fonts.iter().enumerate() {
        if ifc_font_info.identifier == font.identifier() &&
            ifc_font_info.actual_pt_size == font.actual_pt_size
        {
            return index;
        }
    }
    ifc_fonts.push(FontIdentifierAndMetrics {
...
pub(super) fn get_font_for_first_font_for_style(
    ...
    let font = font_context
        ...
        .first(font_context);
    if font.is_none() {
        warn!("Could not find find for style: {:?}", style.clone_font());
    }
    font
}
